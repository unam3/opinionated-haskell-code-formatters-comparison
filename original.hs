...
getSessionName' :: L.Handle a -> WAI.Handle a -> Request -> IO (Either String String)
...
data DBSessionNameAndSessionThings =
...
prerequisitesCheck :: L.Handle a -> S.Handle -> Request -> String -> IO (Either String DBSessionNameAndSessionThings)
...
processCredentials debug clearSessionPartial' request maybeUserId' sessionInsert' wrappedSessionResults = do
    ...
    (do when (isJust maybeUserId') (clearSessionPartial' request))
...
runDBSession loggerH waiH dbH request (DBSessionNameAndSessionThings sessionName' maybeUserId' sessionUserIdString' sessionAuthorIdString' _ sessionInsert' clearSessionPartial') = do
    let runSessionResults = do
            ...
            case eitherConnection of
                Left connectionError ->
                    liftIO $ L.hError loggerH (show connectionError) >> pure (PR.H $ Left $ Right dbError)
                Right connection ->
    ...
    pure $
        case runSessionResultsUnpacked of
...
makeApplication loggerH dbConnectionSettings connectInfo = do
    ...
    simpleConnection <- connectPostgreSQL (postgreSQLConnectionString connectInfo) >>= fromSimpleConnection
                 ...
                 DBC.withDBConnection
                     ...
                     (\dbH ->
                          WAI.withWAI
                              ...
                              (\waiH -> Static.router (S.hWithSession sessionsH $ restAPI loggerH sessionsH dbH waiH))))
...
runWarpWithLogger = do
    L.withLogger
        (L.Config
             ...
             DEBUG
             (traplogging "rest-news" ERROR "Unhandled exception occured" . updateGlobalLogger "rest-news" . setLevel)
             (debugM "rest-news")
             (infoM "rest-news")
             (errorM "rest-news"))
        (\loggerH ->
             C.parseConfig "config.ini" >>= \case
                 ...
                 Right config ->
                     let (port, dbConnectionSettings, connectInfo) = processConfig config
                      in makeApplication loggerH dbConnectionSettings connectInfo >>= run port)
    pure ()
